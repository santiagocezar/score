...
import { BoardStorage, PlayerID, useBoard, createFacet, PlayerInfo, BoardExtension } from 'lib/bx';
import { properties as untypedProperties } from './hasbro_argentina.json';
import { createFacetSerde } from '../facet';

const hasbroArgentinaProperties = new Map<number, MonopolyProperty>();
for (let p of untypedProperties) {
    hasbroArgentinaProperties.set(p.id, p as MonopolyProperty);
}
...
export const MoneyData = createFacet('money', () => 0);
export const PropertiesData = createFacetSerde<Set<number>, number[]>('props', () => new Set(), {
    ser: (v) => Array.from(v),
    des: (v) => (Array.isArray(v) && new Set(v)) || new Set(),
});
